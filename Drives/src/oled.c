...
OLED_OperatorStruct OLED_Opts;
OLED_OperatorStruct_P OLED_Opts_Ptr = &OLED_Opts;
...
void OLED_ShowString_Plus(uint8_t col, uint8_t row, const uint8_t *p)
{
	if((row > 4) || (col > 16))
	{
		return;
	...
	OLED_ShowString((col * 8) - 1, row * 16, p);
}
...
{ 	
	...
	OLED_Opts_Ptr->OLED_Clear = OLED_Clear;
	OLED_Opts_Ptr->OLED_Refresh_Gram = OLED_Refresh_Gram;
	OLED_Opts_Ptr->OLED_Display_On = OLED_Display_On;
	OLED_Opts_Ptr->OLED_Display_Off= OLED_Display_Off;
	OLED_Opts_Ptr->OLED_ShowChar = OLED_ShowChar;
	OLED_Opts_Ptr->OLED_ShowNum = OLED_ShowNum;
	OLED_Opts_Ptr->OLED_ShowxNum = OLED_ShowxNum;
	OLED_Opts_Ptr->OLED_ShowString = OLED_ShowString;
	OLED_Opts_Ptr->OLED_ShowString_Plus = OLED_ShowString_Plus;
